let param = require('minimist')(process.argv.slice(2))
...
switch(param['equation']) {
    ...
    case "etc_estimate":
        ...
        break;
    case "et":
        param_check(param["E"],param["D"],param["d"])
        et(param["E"],param["D"],param["d"]);
        break;
    case "cum_et":
        param_check(param["E"],param["D"],param["d"])
        cum_et(param["E"],param["D"],param["d"]);
        break;
    case "ifft":
        param_check(param["E"],param["D"],param["N"])
        ifft(param["E"],param["D"],param["N"]);
        break;
    case "fft":
        param_check(param["et"])
        fft(param["et"])
        break;
    case "sample2":
        param_check(param["sample"])
        sample2(param["sample"])
        break;
    default:
        param_check(param["sample"])
        process.stdout.write(String(sample(param["sample"])));

}
    function param_check() {
        let args = Array.from(arguments);
        const check = (element) => element == undefined;

        if(Object.values(args).some(check))
            throw new Error("One of the parameters for your functions does not exist");
    ...
    function integer_check() {
        ...
        if(Object.values(args).some(check))
            throw new Error("One of the parameters is not an integer");
    ...
    function etc_estimate(){
        integer_check()

    }

    function cum_et(E,D,d) {
        integer_check(E,D,d)

        let ang=2*Math.PI*(d/365);
        let d1=ang-2*Math.PI*(D[1])/365;
        let d2=ang-2*Math.PI*(D[2]-D[1])/365;
        let et=E[0]*d-(365/2*Math.PI)( E[1]*Math.sin(d1)
                      +E[2]*Math.sin(d1));
        return et;
    }
    ...
    function et(E,D,d) {
        ...
        let d1=2*Math.PI*(d-D[1])/365;
        let d2=(2*2*Math.PI/365)*((d-D[1])-D[2]);
        let et=E[0]+E[1]*Math.cos(d1)+E[2]*Math.cos(d2);
        return et;
    }
